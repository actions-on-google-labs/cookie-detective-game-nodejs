...
const initPlugins = () => {
	...
	const cookie = new Voice({
		...
		speakingRate: 1.15,
		pitch: 8,
		apiKey: configManager.getVariable(VariableNames.TTS_API_KEY)[process.env.NODE_ENV],
		baseUrl: configManager.getVariable(VariableNames.TTS_URL)[process.env.NODE_ENV],
	});
	...
	Vue.use(VueExposePlugin, {
		...
		$hidingSpotModel: new DataModel<THidingSpot>(HidingSpots as Array<THidingSpot>),
	...
	// set up assetManager
	Vue.use(assetManager, { root: configManager.getVariable(VariableNames.VERSIONED_STATIC_ROOT) });
};
