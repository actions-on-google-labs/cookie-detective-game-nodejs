...
export default class SpriteSequence extends EventDispatcher {
	...
	public setup(canvas?: HTMLCanvasElement): void {
		this.canvas =
			canvas || <HTMLCanvasElement>(<HTMLElement>this.options.element).querySelector('canvas');
	...
	protected init(options: ISpriteSequenceOptions): Promise<void> {
		...
		this.frame = this.direction === Direction.FORWARD ? 0 : this.options.frameCount;
		return this.load();
	}
	...
	public restart(direction: Direction = Direction.FORWARD, speed: number = 1): Promise<void> {
		this.frame =
			this.direction === Direction.FORWARD ? this.options.startCount : this.options.frameCount;
		this.tween = null;
	 ...
	 * @method play
	 */
	public play(direction: Direction = Direction.FORWARD, speed: number = 1): Promise<void> {
		return new Promise(resolve => {
			...
			this.stopped = false;
			this.tween = TweenMax.to(this, (frames / this.options.fps) * (1 / Math.max(0.01, speed)), {
				frame: direction === Direction.FORWARD ? this.options.frameCount : this.options.startCount,
	...
	public drawFrame(frame: number = this.frame): void {
		...
		if (image) {
			...
			const mappedFrame = this.options.mapping ? this.options.mapping[frame] : frame;
			const currentColumn = mappedFrame % this.options.columns;
			const currentRow = Math.floor(mappedFrame / this.options.columns);

			const sx =
				currentColumn * (this.options.size.width + this.options.gutter.x) + this.options.gutter.x;
			const sy = currentRow * (this.options.size.height + this.options.gutter.y);
