...
const getAudioContext = () => _audioContext || (_audioContext = new AudioContext());
...
const error = (url: string, msg: string) => new Error(`AssetManager: ${url} - ${msg}`);

const retrieve = (url: string, responseType: XMLHttpRequestResponseType): Promise<any> =>
...
const loadAssetType: { [key: string]: (url: string) => Promise<any> } = {
	...
	VIDEO: (url: string) => retrieve(url, 'blob').then(data => URL.createObjectURL(data)),
	IMAGE: (url: string) =>
		new Promise((resolve, reject) => {
...
const getAssetData = (asset: TAsset): TAssetData =>
	(asset instanceof HTMLImageElement && (asset as HTMLImageElement).src) || asset;

const loadAsset = (url: string, namespace: string = GLOBAL_NAMESPACE): Promise<TAssetData> => {
	...
	if (!assetType) return Promise.reject(error(url, `Unsupported asset type (${extension})`));
...
function load(assets: string[], onProgress?: TProgressCallback): Promise<TAssetData[]>;
function load(
	asset: string,
	namespace?: string,
	onProgress?: TProgressCallback,
): Promise<TAssetData[]>;
function load(asset: string, onProgress?: TProgressCallback): Promise<TAssetData[]>;
function load(
