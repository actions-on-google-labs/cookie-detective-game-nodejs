...
import BackgroundConfettiTransitionController from './BackgroundConfettiTransitionController';
...
export default {
	...
	methods: {
		startConfetti() {
			...
			const start = index => {
				if (this.isPlaying) {
					...
					this.flake[index].y = randomNumber(window.innerHeight * -0.4, window.innerHeight * -0.7);
					...
					this.flakeTweens[index] = TweenLite.to(
						...
						{
							y: window.innerHeight + window.innerHeight * 0.2 + this.flake[index].height,
		...
		handleAllComponentsReady() {
			this.transitionController = new BackgroundConfettiTransitionController(this);
			this.isReady();
		},
	},
};
