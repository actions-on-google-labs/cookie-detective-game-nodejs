...
export default class AnswerOptionsTransitionController extends AbstractTransitionController {
	...
	protected setupTransitionInTimeline(
		...
		this.getButtons(parent).forEach((component, index: number) => {
			...
			timeline.add(this.getTimeline(component, TransitionDirection.IN), index * 0.2);
			buttonsDuration += component.transitionController.transitionInTimeline.duration();
		});

		const refreshButton = <IAbstractTransitionComponent>parent.$refs.refreshButton;
		if (refreshButton) {
			timeline.add(
				this.getTimeline(refreshButton, TransitionDirection.IN),
				timeline.duration() - refreshButton.transitionController.transitionInTimeline.duration(),
			);
		}
	}
	...
	protected setupTransitionOutTimeline(
		...
		buttons.forEach((component, index: number) => {
			...
			timeline.add(this.getTimeline(component, TransitionDirection.OUT), index * 0.1);
		});

		const refreshButton = <IAbstractTransitionComponent>parent.$refs.refreshButton;
		if (refreshButton) {
			timeline.add(this.getTimeline(refreshButton, TransitionDirection.OUT), timeline.duration());
		}
	}
