...
export default class LevelPreviewCardTransitionController extends AbstractTransitionController {
	...
	protected setupTransitionInTimeline(
		...
		if (parent.$refs.leaderboardItem) {
			timeline.add(this.getTimeline('leaderboardItem', TransitionDirection.IN), '-=0.4');
		}
	}
	...
	public triggerLockedLevelAnimation(): void {
		...
		timeline.to(currentCardElement, 0.15, { x: -20, ease: Power2.easeInOut }, 0.5);
		...
		const lockIcon = (<IAbstractTransitionComponent>this.parentController.$refs.lock).$el;
		...
		const button = (<IAbstractTransitionComponent>this.parentController.$refs.button).$el;
