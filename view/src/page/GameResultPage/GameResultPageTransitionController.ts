...
export default class GameResultPageTransitionController extends AbstractTransitionController {
	...
	public setupDanceTimeline(): void {
		...
		const endDisco = crossPageTransitionWaitTime + (gameResult.isGameOver() ? 0 : 6);
		...
		if (!gameResult.isGameOver()) {
			...
			this.danceTimeline.add(() => {
				(<IAbstractTransitionComponent>this.parentController.$refs.discoLights).transitionOut();
			}, endDisco);

			this.danceTimeline.fromTo(
				(<IAbstractTransitionComponent>this.parentController.$refs.lottieAnimation).$el,
				1,
			...
			this.danceTimeline.add(this.getTimeline('lottieAnimation', TransitionDirection.IN), 0.1);

			this.danceTimeline.add(() => {
				(<any>this.parentController.$refs.confettiExplosion).play();
				(<any>this.parentController.$refs.confettiExplosionRight).play();
			}, startCharacter);
		} else {
			this.danceTimeline.fromTo(
				(<IAbstractTransitionComponent>this.parentController.$refs.lottieAnimation).$el,
			...
			this.danceTimeline.add(this.getTimeline('lottieAnimation', TransitionDirection.IN), 1);
		}
	...
	public setupHighScoreTimeline(): void {
		...
		this.highScoreTimeline.add(this.getTimeline('gameResultCard', TransitionDirection.IN), 0.4);
	...
	public showButtons(): void {
		...
		if (ctaLeft && ctaLeft.transitionController.isHidden) {
			(<IAbstractTransitionComponent>this.parentController.$refs.ctaButtonLeft).transitionIn();
		}
		...
		if (ctaRight && ctaRight.transitionController.isHidden) {
			TweenLite.delayedCall(0.2, () => {
				(<IAbstractTransitionComponent>this.parentController.$refs.ctaButtonRight).transitionIn();
			});
		}
	}
	...
	protected setupTransitionOutTimeline(
		...
		if (ctaLeft && !ctaLeft.transitionController.isHidden) {
			timeline.add(this.getTimeline('ctaButtonLeft', TransitionDirection.OUT), 0);
		}

		const ctaRight = this.getComponent('ctaButtonRight');

		if (ctaRight && !ctaRight.transitionController.isHidden) {
			timeline.add(this.getTimeline('ctaButtonRight', TransitionDirection.OUT), 0.1);
		}
	}
	...
	public showUnlockedLevel(): Promise<void> {
		const resultCard = <IAbstractTransitionComponent>this.parentController.$refs.gameResultCard;
