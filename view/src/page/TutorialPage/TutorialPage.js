...
import TutorialPageTransitionController from './TutorialPageTransitionController';
...
export default {
	...
	created() {
		...
		this.voices = {
			...
			intro_yellow_fallback: this.$tFormatted('voice.intro_yellow'),
			intro_hidingspot_jar: this.$tFormatted('voice.intro_hidingspot.jar'),
			intro_hidingspot_cupboard: this.$tFormatted('voice.intro_hidingspot.cupboard'),
	...
	methods: {
		...
		async showWelcomeText() {
			...
			await Promise.all([this.$say(this.voices.intro_1), this.$refs.question.transitionIn()]);
		...
		async showQuestionInstructionText() {
			...
			await Promise.all([
				...
				this.hideAndShowNewBubble(this.$t(`introduction.questionInstructionText`)),
			]);
