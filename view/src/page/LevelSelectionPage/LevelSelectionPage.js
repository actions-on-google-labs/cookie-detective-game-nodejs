...
import LevelSelectionPageTransitionController from './LevelSelectionPageTransitionController';
import LevelPreviewCard from '../../component/general/content/LevelPreviewCard/LevelPreviewCard';
import BackgroundConfetti from '../../component/asset/BackgroundConfetti/BackgroundConfetti';
import LeaderboardItem from '../../component/asset/LeaderboardItem/LeaderboardItem';
import PrimaryTextReveal from '../../component/type/PrimaryTextReveal/PrimaryTextReveal';
...
export default {
	...
	created() {
		...
		this.levels.forEach(level => {
			...
			this.voices[`locked_level_${level.id}`] = this.$tFormatted('voice.start_locked_level', {
				level: name,
			});
		});
	},
	...
	methods: {
		handleVoiceInput(input) {
			const levelIndex = this.levels.findIndex(level => level.ordinal === input);
		...
		async goToLevel(level, index) {
			...
			if (this.isLevelUnlocked(level, this.user)) {
				otherCardIndexes.forEach(int => this.$refs.cards[int].transitionOut());
		...
		handleAllComponentsReady() {
			this.transitionController = new LevelSelectionPageTransitionController(this);
