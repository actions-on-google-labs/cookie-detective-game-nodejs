	...
	protected setupTransitionInTimeline(
		...
		(<Array<IAbstractTransitionComponent>>parent.$refs.cards).forEach(
			(component, index: number) => {
				timeline.add(this.getTimeline(component, TransitionDirection.IN), index * 0.6 + 0.4);
			},
		);
	}

	public moveCardToCenter(index: number = 0): Promise<void> {
		const cards = <Array<IAbstractTransitionComponent>>this.parentController.$refs.cards;
		const currentCard = <HTMLElement>cards[index].$el;
		const cardWidth = currentCard.offsetWidth;
		const spaceBetweenCard =
			(<HTMLElement>cards[1].$el).offsetLeft - ((<HTMLElement>cards[0].$el).offsetLeft + cardWidth);
		...
		return new Promise((resolve: Function) => {
			...
			this.outroTimeline.add(this.getTimeline('title', TransitionDirection.OUT), 0);
			this.outroTimeline.set(currentCard, { zIndex: 1, pointerEvents: 'none' }, 0);
